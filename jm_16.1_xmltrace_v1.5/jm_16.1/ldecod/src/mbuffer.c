...
//       error ("max_dec_frame_buffering larger than MaxDpbSize", 500);
      warning("max_dec_frame_buffering larger than MaxDpbSize, clamping", 0);
      active_sps->vui_seq_parameters.max_dec_frame_buffering = size;
...
void init_dpb(VideoParameters *p_Vid)
{
  ...
  if (p_Dpb->size < active_sps->num_ref_frames)
  {
    //error ("DPB size at specified level is smaller than the specified number of reference frames. This is not allowed.\n", 1000);
     warning("DPB size at specified level is smaller than the specified number of reference frames, clamping num_ref_frames.", 0);
     active_sps->num_ref_frames = p_Dpb->size;
  }
